...
class WrittenGrantMetaAnalysis:
    ...
    def _get_files(self):
        logging.info("Getting files from grant directory")
        files = [f for f in os.listdir(self.grant_dir) if f.endswith('.md')]
    ...
    def _read_files(self):
        ...
        for file in self.files:
            logging.info(f"Reading file: {os.path.join(self.grant_dir, file)}")
            with open(os.path.join(self.grant_dir, file), 'r', encoding='utf-8') as f:
                texts[file] = f.read()
    ...
    def visualize_projections(self, coords, method):
        ...
        grants = [file.split('_')[1].split('.md')[0] if len(file.split('_')) > 1 else 'Unknown' for file in self.files]
    ...
    def pca_interpretability(self, pca, vectorizer):
        ...
        for i, component in enumerate(pca.components_[:n_components]):
            ...
            plt.barh(range(n_top_terms), loadings, align='center')
            plt.yticks(range(n_top_terms), terms)
            plt.title(f'Top Terms for PC{i+1}')
            plt.xlabel('Loading')
        
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, 'PCA_Analysis', 'top_terms_per_component.png'), dpi=300, bbox_inches='tight')
        plt.close()
...
if __name__ == "__main__":
    analyzer = WrittenGrantMetaAnalysis()
