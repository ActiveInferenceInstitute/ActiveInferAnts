...
def generate_concatenated_domains(domains: List[str], prompt: str, output_dir: str) -> None:
    ...
    Args:
        domains (List[str]): List of domain contents.
    ...
    for domain_a, domain_b in product(domains, repeat=2):
        ...
        save_file(concatenated_content, output_dir, output_filename)
...
def process_domains(config: dict = DEFAULT_CONFIG) -> None:
    ...
    try:
        ...
        config['output_dirs'] = {
            ...
            'Concatenated_Domain': os.path.join(base_dir, 'Pro-Shifted_Domain')
        }
